...
// Dimensions label (W × H pill near cursor). Fixed width avoids jitter; wide
// enough for two 4-digit dimensions ("3840 × 2160") at the label font below.
const LABEL_WIDTH: i32 = 120;
const LABEL_HEIGHT: i32 = 26;
// Fixed pixel height for the label font. A stock font scales with the primary
// monitor's DPI (≈24px at 200%) and overflows the fixed-px pill; a fixed height
// keeps the text inside the box at any DPI.
const LABEL_FONT_HEIGHT: i32 = 16;
    ...
    unsafe {
        ...
        if let Some(s) = sel {
            ...
            if sw > 0 && sh > 0 {
                ...
                // 1px white outline on the dim/bright boundary so the selection
                // edge reads clearly even over bright content (the window is
                // opaque, so this is a crisp solid line).
                let edge = CreateSolidBrush(COLORREF(0x00FF_FFFF));
                if !edge.is_invalid() {
                    let _ = FrameRect(state.work_dc, &raw const s, edge);
...
unsafe fn paint_label(hdc: HDC, hwnd: HWND) {
    ...
    unsafe {
        ...
        // Fixed-height Segoe UI (not the DPI-scaled stock font, which overflows
        // the fixed-px pill on high-DPI monitors).
        ...
        let prev_font = SelectObject(hdc, font.into());
